...
def flatten(s):
    ...
    def flatten_to_one_dim(src_lst):
        """返回一维列表"""
        flatten_list = []
        for x in src_lst:
            # result可能为一维列表或单个值
            ...
            # 对递归后数据收集做判断
            if is_list(result):
                flatten_list.extend(result)
            else:
                flatten_list.append(result)
        return flatten_list

    # 对当前的s做判断
    if is_list(s):
        return flatten_to_one_dim(s)
    else:
        return s
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) \
        if distance(city_a, make_city('', lat, lon)) < distance(city_b, make_city('', lat, lon)) \
    ...
    1
      2
        3
          ...
          2
    """
    # copy origin tree and add new trees on leaves
    if is_leaf(t):
